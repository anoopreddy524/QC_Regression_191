...
public class VQCClearMenu extends QCStore {
	...
	{
		try{
			int lastrow=TestData.getLastRow((Aprop.getProperty("Clear")));
			String sheetName=Aprop.getProperty("Clear");
			...
			for(String winHandle : driver.getWindowHandles()){
				  ...
				  if(ProductID.equals(Aprop.getProperty("PDL")))
					 {
					   ...
					   driver.switchTo().defaultContent();
						...
						if(ESign_CollateralType.equalsIgnoreCase("CHECK"))
						{
							Actions action = new Actions(driver);  
							 ...
							 Thread.sleep(2000);
							 WebElement pdl=driver.findElement(locator(Aprop.getProperty("deposite_menu_pdl")));
							 action.moveToElement(pdl).build().perform();
							 test.log(LogStatus.PASS, " Mouse hover on payday loan menu");
							 Thread.sleep(3000);
							 driver.findElement(locator(Aprop.getProperty("deposite_clear_menu"))).click();
							 ...
							 driver.findElement(locator(Aprop.getProperty("deposite_menu_submit"))).click();
						...
						{
							Actions action = new Actions(driver);  
							 WebElement collateral=driver.findElement(locator(Aprop.getProperty("ACH")));
							 action.moveToElement(collateral).build().perform();
							 test.log(LogStatus.PASS, " Mouse hover on ACH menu");
							 Thread.sleep(2000);
							 WebElement pdl=driver.findElement(locator(Aprop.getProperty("deposit_ach_pdl")));
							 action.moveToElement(pdl).build().perform();
							 test.log(LogStatus.PASS, " Mouse hover on payday loan menu");
							 Thread.sleep(3000);
							 driver.findElement(locator(Aprop.getProperty("ACH_clear"))).click();
							 test.log(LogStatus.PASS, " click on clear menu");
							 Thread.sleep(2000);
							 driver.switchTo().defaultContent();
							 driver.switchTo().frame("mainFrame");
